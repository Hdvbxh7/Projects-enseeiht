...
let rec retrait_arbre lc (Noeud (b, lb)) =
   match lc with
      |[] -> Noeud(false,lb)
      |t::qlc->match recherche t lb with
                  |None->Noeud(b,lb)
                  |Some a ->Noeud(b,maj t (retrait_arbre qlc a) lb)

(******************************************************************************)
(*   fonction de parcours d'une liste éléments dans un arbre                   *)
(*   signature  : ajout : 'a arbre -> 'a list list                                 *)
(*   paramètres : - un arbre n-aire                                           *)
(*   résultat   : la liste des elements de l'arbre                             *)
(******************************************************************************)
let rec parcours (Noeud(b,la)) = let parc (t,q) = match q with |Noeud(b2,_) -> if b2 then ([t]::(List.map (fun e -> t::e) (parcours q))) else (List.map (fun e -> t::e) (parcours q)) in
                        if la=[] then [] else List.flatten(List.map (fun e -> parc e) la)
...
let arbre_sujet4 = parcours arbre_sujet
