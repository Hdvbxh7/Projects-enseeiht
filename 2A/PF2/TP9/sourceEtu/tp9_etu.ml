...
and parse_I : (char, inst) parser = fun flux ->
  (
    ...
    (parse_C
    *> p_ptvirg
    *> parse_I >>= fun ( ( c , () ) , i ) -> return (c::i))
  ) flux
and parse_C : (char, cmd) parser = fun flux ->
  (
    ((p_repeat
    *> p_entier
    *> parse_P) >>= fun (((),e),p) -> return (Repeat(e,p)))

    ++

    (p_move
    *> p_entier >>= fun ((),e) -> return (Move e))

    ++

    (p_turn
    *> p_entier  >>= fun ((),e) -> return (Turn e))
...
let run_logo prog =
  ...
  let dessin_f fich =
    let rec dessiner flux =
    match (Solution.uncons flux) with
      |None -> ()
      |Some (t,q) -> (run_logo t); dessiner q
    in
    dessiner (parse_logo (flux_of_file fich))