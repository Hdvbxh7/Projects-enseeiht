...
ruleRead _env _expr mem = 
  let (pval,pmem) =
    value_of_expr (_expr,mem) _env 
   in
  match pval with
   | (ErrorValue _) as result -> (result,mem)
   | ReferenceValue p -> 
    (match lookforMem p pmem with
      | Found(v) -> (v,pmem)
      | NotFound -> ((ErrorValue (UnknownReferenceError p)),mem)
    )
...
ruleWrite _env _refexpr _valexpr mem = 
  let (pval,pmem) =
    value_of_expr (_valexpr,mem) _env 
  in
  let (prefexpr,pmem2) =
    value_of_expr (_refexpr,pmem) _env 
  in
    match prefexpr with
      | ReferenceValue name ->(
        match lookforMem name pmem2 with
        | Found _ -> (NullValue,(name,pval)::pmem2)
        | NotFound -> (ErrorValue (UnknownReferenceError name), pmem)
      )
      | (ErrorValue _) as result -> (result,mem) 
      | _ -> ((ErrorValue TypeMismatchError),mem)
...
ruleSequence _env _left _right mem = 
  let (pleft,pmem) =
    value_of_expr (_left,mem) _env 
  in
   match pleft with 
    | (ErrorValue _) as result -> (result,mem)
    | NullValue ->   let (pright,pmem2) =
    value_of_expr (_right,pmem) _env 
    in (match pright with
      |(ErrorValue _) as result -> (result,mem)
      | _ -> (pright,pmem2)
    )
    | _ -> ((ErrorValue TypeMismatchError),mem) 
...
ruleWhile _env _cond _body mem = 
  let (pcond,pmem) =
    value_of_expr (_cond,mem) _env 
  in
  let (pbody,pmem2) =
    value_of_expr (_body,pmem) _env 
  in 
    match pcond with
    |(ErrorValue _) as result -> (result,mem)
    | BooleanValue false -> (NullValue,pmem)
    | BooleanValue true -> 
     ( match pbody with
      |(ErrorValue _) as result -> (result,mem)
      | NullValue -> value_of_expr (WhileNode(_cond,_body),pmem2) _env
      | _-> ((ErrorValue TypeMismatchError),pmem) )
    | _ -> ((ErrorValue TypeMismatchError),mem) 
and
(* .............................................................................*)
...
ruleReference _env _expr mem = 
  ...
  match pexpr with
  |(ErrorValue _) as result -> (result,mem)
  | c -> let ref = newReference() in 
    (ReferenceValue ref,(ref,c)::pmem)    
