...
let rec insert ordre elt l = match l with | []->[elt] |t::q -> if ordre t elt then t::(insert ordre elt q) else elt::l
...
let rec tri_insertion ordre l = match l with |[]->[] | t1::t2::q -> if ordre t1 t2 then tri_insertion ordre (insert ordre t1 (t2::q)) else t1::tri_insertion ordre (insert ordre t2 q)|t::q-> insert ordre t q
