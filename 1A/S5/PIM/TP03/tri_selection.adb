...
procedure Tri_Selection is
    ...
    procedure Tri_S(F_T: in out T_Tableau) is
        sauver:integer;
        ind_m: integer;
	    begin 
	        for i in 1..F_T.Taille loop
	            ind_m:=i;
	            --chercher minimum
	            for m in i..F_T.Taille loop
	                if F_T.Elements(m)<=F_T.Elements(ind_m) then
	                    ind_m:=m;
	                else
	                    null;
	                end if;
	            ...
	            --permutation
	            if F_T.Elements(ind_m)=F_T.Elements(i) then 
	                null;
	            else
	                sauver:=F_T.Elements(i);
	                F_T.Elements(i):=F_T.Elements(ind_m);
	                F_T.Elements(ind_m):=sauver;
	            end if;
	        end loop;    	                    
	    end Tri_S;
